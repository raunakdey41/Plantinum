...
export default function ProductDetail() {
  ...
  if (!product) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center bg-surface">
        <h1 className="font-headline-lg text-primary mb-4">Plant Not Found</h1>
        <Link href="/shop/indoor-plants" className="px-6 py-3 bg-primary text-on-primary rounded-xl font-bold hover:bg-secondary transition-colors">
          Return to Catalogue
        </Link>
      </div>
    );
  ...
  return (
    <div className="bg-surface min-h-screen pb-space-md">
      {/* Breadcrumbs */}
      <section className="w-full px-gutter-mobile lg:px-margin pt-space-md pb-space-sm bg-surface-container-low/50">
          ...
          <div className="w-full lg:w-1/2 flex flex-col-reverse sm:flex-row gap-4">
            ...
            <div className="flex-1 relative bg-surface-container-lowest rounded-2xl aspect-[4/5] flex items-center justify-center shadow-sm overflow-hidden">
              <button 
                ...
                className="absolute top-4 right-4 z-10 w-10 h-10 rounded-full bg-surface-container-lowest/90 backdrop-blur-sm flex items-center justify-center text-outline hover:text-tertiary transition-colors shadow-sm"
              >
      ...
      {/* Reviews Section */}
      <section id="reviews" className="w-full px-gutter-mobile lg:px-margin py-space-xl bg-surface-container-lowest border-t border-surface-container">
        <div className="max-w-7xl mx-auto">
          <h2 className="font-headline-md text-primary font-bold mb-8">Customer Reviews</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="p-6 bg-surface-container-low rounded-xl">
                ...
                <h4 className="font-bold text-primary mb-2">Beautiful and healthy!</h4>
                <p className="text-on-surface-variant text-sm mb-4">Arrived in perfect condition. The packaging was incredibly secure. It looks gorgeous in my living room.</p>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-secondary-container flex items-center justify-center text-xs font-bold text-secondary">A</div>
                  <span className="text-xs font-medium text-outline">Verified Buyer</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {similarProducts.length > 0 && (
        <section className="w-full px-gutter-mobile lg:px-margin py-space-xl bg-surface">
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between mb-8">
              <h2 className="font-headline-md text-primary font-bold">You May Also Like</h2>
                ...
                return (
                  <article key={p.id} className="group bg-surface-container-lowest rounded-xl shadow-sm hover:shadow-md transition-all duration-300 flex flex-col overflow-hidden">
                    <div className="relative aspect-square w-full bg-surface-container-low overflow-hidden flex items-center justify-center p-3">
                      ...
                      <Link href={`/shop/indoor-plants/${p.id}`} className="w-full h-full block">
                    ...
                    <div className="p-3.5 flex-1 flex flex-col justify-between">
                      <div>
                        <Link href={`/shop/indoor-plants/${p.id}`}>
