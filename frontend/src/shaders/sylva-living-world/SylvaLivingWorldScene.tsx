...
import innerGreenSource from "./sources/inner-green-3d.html.js";
import threeRuntime from "./sources/inner-green-assets/three.min.js.js";
...
export const LIVING_GREEN_STYLE = `<style data-threeui-sylva-living-green>
html,
body {
...
.hero {
  background:
    radial-gradient(64% 52% at 27% 84%, rgba(255,255,255,0.4) 0%, rgba(255,255,255,0) 72%),
    radial-gradient(70% 60% at 92% 8%,  rgba(220,230,215,0.4) 0%, rgba(220,230,215,0) 68%),
    #eaf0e6 !important;
}

.hero::after {
  background:
    radial-gradient(72% 44% at 50% 117%, rgba(255,255,255,0.7) 0%, rgba(255,255,255,0.3) 42%, rgba(255,255,255,0.05) 72%, rgba(255,255,255,0) 88%),
    linear-gradient(180deg, rgba(255,255,255,0) 54%, rgba(255,255,255,0.1) 78%, rgba(255,255,255,0.2) 100%) !important;
}
</style>`;
...
const VARIANT_STYLES: Partial<Record<SylvaLivingWorldVariant, string>> = {
  "living-green": LIVING_GREEN_STYLE,
...
export function applyMapleAutumnVariant(source: string) {
  ...
  var FAR_N  = { w:  920, left:  280, top:  902, aspect: 1600 /  757, z: -260 };`,
    ...
    ["place(nearGroup, A, 0.732, 0.06, 0);", "place(nearGroup, A, 0.500, 0.500, 0);", "tree framing"],
    ["place(farGroup,  F, 0.410, 0.32, F.z);", "place(farGroup,  F, 0.500, 0.500, -100);", "grove framing"],
...
export function applyLivingGreenVariant(source: string) {
  const replacements: Array<[string, string, string]> = [
    ["uKeyCol:  { value: new THREE.Color(1.14, 1.06, 0.88) },", "uKeyCol:  { value: new THREE.Color(2.28, 2.12, 1.76) },", "brighter key light"],
    ["uAmbCol:  { value: new THREE.Color(0.086, 0.090, 0.080) },", "uAmbCol:  { value: new THREE.Color(0.25, 0.28, 0.22) },", "brighter ambient light"],
    ["renderer.toneMappingExposure = 1.30;", "renderer.toneMappingExposure = 1.65;", "brighter exposure"],
    ["hazeCol: [0.150, 0.164, 0.120], hazeLift: 0.92,", "hazeCol: [0.85, 0.90, 0.85], hazeLift: 0.05,", "lighter sky haze"],
    ["uHazeCol: { value: new THREE.Color(0.176, 0.195, 0.145) },", "uHazeCol: { value: new THREE.Color(0.85, 0.90, 0.85) },", "lighter fog color"],
    [
      "scene = new THREE.Scene();",
      `scene = new THREE.Scene();
      window.wfTime = 0;
      
      // Expand plane to 3000 to fill the ENTIRE animated section with fog
      const wfGeo = new THREE.PlaneGeometry(3000, 500, 64, 32);
      ...
      const wfMat = new THREE.ShaderMaterial({
        ...
        vertexShader: \`
          ...
          void main() {
            ...
            // Distort the bottom to simulate a splash/crash, restricted to the center waterfall
            float bottom = 1.0 - uv.y;
            // Center mask for the splash (only the waterfall part, roughly center 10-15%)
            float centerMask = smoothstep(0.42, 0.44, uv.x) * smoothstep(0.58, 0.56, uv.x);
            pos.z += smoothstep(0.7, 1.0, bottom) * 50.0 * centerMask;
            pos.x += pos.x * smoothstep(0.7, 1.0, bottom) * 0.3 * centerMask;
        ...
        fragmentShader: \`
          ...
          void main() {
            ...
            // --- WATERFALL (CENTER) ---
            // UV scaling to account for the super wide plane (3000)
            vec2 flowUv = vUv * vec2(40.0, 3.0) + vec2(0.0, time);
            float n1 = fbm(flowUv);
            vec2 streakUv = vUv * vec2(60.0, 1.0) + vec2(0.0, time * 1.5);
            float n2 = fbm(streakUv);
            float water = (n1 * 0.7 + n2 * 0.3);
            
            // Greyish blue slate colors (matching the requested shade)
            vec3 darkWater = vec3(0.25, 0.35, 0.45);
            vec3 midWater = vec3(0.45, 0.55, 0.65);
            vec3 brightFoam = vec3(0.85, 0.90, 0.95);
            
            vec3 waterCol = mix(darkWater, midWater, smoothstep(0.35, 0.55, water));
            waterCol = mix(waterCol, brightFoam, smoothstep(0.55, 0.8, water));
            
            // Waterfall occupies a narrow slice in the center (approx 300-400 width out of 3000)
            float waterfallMask = smoothstep(0.43, 0.45, vUv.x) * smoothstep(0.57, 0.55, vUv.x);
            
            // --- FOG & MIST (ENTIRE SPACE) ---
            // Slower, billowy noise for fog covering the entire plane
            // Multiplied time to make it move noticeably faster
            float fogN = fbm(vUv * vec2(12.0, 3.0) + vec2(time * 0.25, time * 0.15));
            // Darker slate mist so it contrasts visibly against the bright sky
            vec3 mistyGrey = vec3(0.4, 0.5, 0.55);
            vec3 paleMist = vec3(0.65, 0.75, 0.8);
            vec3 fogCol = mix(mistyGrey, paleMist, smoothstep(0.3, 0.7, fogN));
            
            // --- COMBINE ---
            vec3 finalCol = mix(fogCol, waterCol, waterfallMask);
            
            float waterAlpha = smoothstep(0.25, 0.6, water) * waterfallMask;
            // Increased fog alpha significantly to make it very vivid (max 0.75)
            float fogAlpha = smoothstep(0.2, 0.8, fogN) * 0.75; 
            
            float combinedAlpha = max(waterAlpha, fogAlpha);
            
            // Edge fading masks so the plane doesn't show hard borders
            float outerFade = smoothstep(0.0, 0.1, vUv.x) * smoothstep(1.0, 0.9, vUv.x);
            float topMask = smoothstep(1.0, 0.92, vUv.y);
            float bottomMask = smoothstep(0.0, 0.15, vUv.y);
            float boundsMask = outerFade * topMask * bottomMask;
            
            gl_FragColor = vec4(finalCol, combinedAlpha * boundsMask * 0.9);
          }
        \`
      });
      
      window.wfMesh = new THREE.Mesh(wfGeo, wfMat);
      
      // Center Y = 0, Height = 500 means Top = 250, Bottom = -250
      window.wfMesh.position.set(0, -10, -280);
      window.wfMesh.renderOrder = -1;
      
      scene.add(window.wfMesh);`,
      "init waterfall mesh"
    ],
    [
      "function tick() {",
      `function tick() {
        if(window.wfMesh) {
          window.wfTime += 0.016;
          window.wfMesh.material.uniforms.uTime.value = window.wfTime;
        }`,
      "tick waterfall mesh"
    ],
  ];
...
function buildSceneDocument(reducedMotion: boolean, variant: SylvaLivingWorldVariant) {
  ...
  if (variant === "living-green") documentSource = applyLivingGreenVariant(documentSource);
